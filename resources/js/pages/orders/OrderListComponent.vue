<template>
  <div class="p-6 space-y-6">
    ...
    <OrderDetailComponent v-model:order="selectedOrder" />
  </div>
</template>
...
const page         = ref(1);
const filters      = ref({ date_from: '', date_to: '', payment_method: 'all' });

async function fetchOrders() {
  loading.value    = true;
  const params     = { page: page.value, ...filters.value, payment_method: filters.value.payment_method === 'all' ? '' : filters.value.payment_method };
